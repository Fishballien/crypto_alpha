...
from utils.datautils import get_one_factor, load_all_features_of_factors, load_one_group_of_features, align_index_with_main, replace_sp
...
class ModelFit(ABC):
    ...
    def _load_public_paths(self):
        ...
        self.processed_data_dir = Path(self.path_config['processed_data'])
        
    def _init_dir(self):
        pool_name = self.preprocess_params['pool_name']
        
        self.data_dir = self.processed_data_dir / 'features_of_factors'
        self.cluster_dir = self.result_root_dir / 'features_of_factors' / pool_name / 'cluster'
        self.result_dir = self.result_root_dir / 'features_of_factors' / pool_name / 'model' / self.test_name
    ...
    def _load_n_preprocess_x_y(self, start_date, end_date):
        predict_target_data = self._load_n_preprocess_to_predict(start_date, end_date)
        factor_data = self._load_n_preprocess_factors(start_date, end_date, 
                                                      ref_order_col=predict_target_data.columns,
                                                      ref_index=predict_target_data.index)
        predict_target_ravel = predict_target_data.values.reshape(-1, order='F')
        ...
        y = predict_target_ravel[~self.to_mask_ravel]
        # if self.mode == 'predict':
        # breakpoint()
        return X, y, predict_target_data
    
    @gc_collect_after
    def _load_n_preprocess_to_predict(self, start_date, end_date):
        pool_name = self.preprocess_params['pool_name']
        predict_target = self.preprocess_params.get('predict_target')
        
        path = self.data_dir / pool_name / f'{predict_target}.parquet'
        predict_target_data = pd.read_parquet(path)
        
        predict_target_data = predict_target_data[
            (predict_target_data.index >= start_date) & (predict_target_data.index < end_date)]
        self.predict_target_data = predict_target_data
        self.to_mask = predict_target_data.isna() # !!!：应该没有，为了不改函数，先写着
        predict_target_data = self.normalization_func(predict_target_data)
        predict_target_data = predict_target_data.fillna(0)
...
#         
#         get_one_factor_func = partial(get_one_factor,
#                                       date_start=start_date, date_end=end_date,
#                                       ref_order_col=ref_order_col, ref_index=ref_index)
#         load_one_group_func = partial(load_one_group_of_features, get_one_factor_func=get_one_factor_func,
...
# =============================================================================
    ...
    def _load_n_preprocess_factors(self, start_date, end_date, ref_order_col, ref_index):
        ...
        period_suffix = self.model_period_suffix if self.mode == 'fit' else self.predict_suffix
        dataset_path = self.data_dir / self.pool_name / f'dataset_{cluster}_{period_suffix}.pkl'
        if os.path.exists(dataset_path):
            with open(dataset_path, 'rb') as f:
                dataset = pickle.load(f)
            factor_arr = dataset['factor_arr']
            self.features = dataset['group_names']
            return factor_arr
        ...
        get_one_factor_func = partial(get_one_factor, factor_data_dir=self.data_dir, 
                                      date_start=start_date, date_end=end_date,
                                      ref_order_col=ref_order_col, ref_index=ref_index)
        
        factor_dict = load_all_features_of_factors(cluster_info, get_one_factor_func, self.n_workers)
        
        load_one_group_func = partial(load_one_group_of_features, factor_dict=factor_dict,
                                      normalization_func=self.normalization_func,
    ...
    def test_predicted(self):
        ...
        predict_target_dir = self.data_dir / pool_name
        
        ft = TestFeatures(pool_name, factor_data_dir, result_dir, predict_target_dir=predict_target_dir)
        ft.test_one_feature(f'predict_{self.test_name}', check_exists=False)
...
class EnsembleModelFit(ModelFit):
    ...
    def _init_train_params(self, y_data):
        ...
        objective_func = partial(objective_org_func, rtn=self.predict_target_data, 
