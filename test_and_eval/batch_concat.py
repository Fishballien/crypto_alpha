...
def load_test_data(factor_name, data_dir):
    ...
    df_hsr = pd.read_parquet(data_dir / f'hsrts_{factor_name}.parquet')
    try:
...
def save_test_data(factor_name, data_dir, data_dict):
    ...
    data_dict['hsrts'].to_parquet(data_dir / f'hsrts_{factor_name}.parquet')
...
def merge_common_keys(factor_name, updater, dict1, dict2, parent_key_chain=None):
    # 初始化键链
    ...
    for key in common_keys:
        ...
        if isinstance(dict1[key], dict) and isinstance(dict2[key], dict):
            ...
            merged_dict[key] = merge_common_keys(factor_name, updater, dict1[key], dict2[key], 
                                                 current_key_chain) # HINT: 注意在递归，函数该参数需要改这里
        elif isinstance(dict1[key], pd.DataFrame) and isinstance(dict2[key], pd.DataFrame):
            # 如果两者都是 DataFrame，打印当前键链并拼接
            key_chain_name = f"{factor_name} {' -> '.join(current_key_chain)}"
...
class BatchConcat:
    ...
    def _init_dir(self):
        self.param_dir = Path(self.path_config['param']) / 'batch_concat'
        self.test_dir = Path(self.path_config['result']) / 'test'
        
    def _load_params(self):
        self.params = toml.load(self.param_dir / f'{self.batch_concat_name}.toml')
...
# =============================================================================
    ...
    def _concat_by_directory(self):
        ...
        for root_dir in mapping:
            ...
            for process_name in process_name_list:
                kwargs = {f'{tag_type}_dir': tag_dir_mapping[tag_type] / process_name / 'data'
                          for tag_type in tag_dir_mapping}
                kwargs.update({'process_name': process_name, 'root_dir': root_dir})
                self._concat_one_process(kwargs)
    ...
    def _concat_one_process(self, kwargs):
        ...
        updated_exists = os.path.exists(kwargs['updated_dir'])
        if not updated_exists:
            kwargs['updated_dir'].mkdir(parents=True, exist_ok=True)
        
        factor_dir = Path(kwargs['root_dir']) / kwargs['process_name']
        ...
        for factor_name in tqdm(factor_name_list, desc=kwargs['process_name']):
            try:
                his_dir = kwargs['updated_dir'] if updated_exists else kwargs['his_dir']
                his_test = load_test_data(factor_name, his_dir)
            except:
                self.log.warning(f"Failed to fetch {factor_name} from {kwargs['his_dir']}")
                continue
            try:
                inc_test = load_test_data(factor_name, kwargs['inc_dir'])
            except:
                self.log.warning(f"Failed to fetch {factor_name} from {kwargs['inc_dir']}")
                continue
            updated_test = merge_common_keys(factor_name, self.updater, his_test, inc_test)
            self._get_hsr_by_month(updated_test)
            save_test_data(factor_name, kwargs['updated_dir'], updated_test)
        
        self.log.success(f"Finished concating {kwargs['process_name']}")
        
    def _get_hsr_by_month(self, updated_test):
        hsrts = updated_test['hsrts']
        hsrm = hsrts.resample('M').mean()
        updated_test['hsr'] = hsrm
