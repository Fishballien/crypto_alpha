...
class FactorTest:
    ...
    def __init__(self, process_name, tag_name, factor_data_dir, test_name='', result_dir=None, params=None,
                 skip_plot=False, n_workers=1, date_start=None, date_end=None):
    ...
    def _preprocess_twap_return(self):
        ...
        rtn_of_lag = {}
        for lag in lag_list:
            rtn_lag = rtn_1p.shift(-lag*pp_by_sp).replace([np.inf, -np.inf], np.nan)
            twap_to_mask = twap_to_mask | rtn_lag.isna()
            rtn_of_lag[lag] = rtn_lag.loc[self.date_start:self.date_end]
        
        # pp
        pp_list = [pp for pp in pp_list if pp % sp_in_min == 0]
        rtn_of_pp = {}
        for pp_ in pp_list:
            pp_by_sp_ = int(pp_ / sp_in_min)
            rtn_pp = twap_price.pct_change(pp_by_sp_, fill_method=None).shift(-pp_by_sp_).replace([np.inf, -np.inf], np.nan)
            twap_to_mask = twap_to_mask | rtn_pp.isna()
            rtn_of_pp[pp_] = rtn_pp.loc[self.date_start:self.date_end]
            
        self.twap_to_mask = twap_to_mask.loc[self.date_start:self.date_end]
        self.sp_in_min = sp_in_min
        self.pp_by_sp = pp_by_sp
        self.pp_list = pp_list
        self.rtn_1p = rtn_1p.loc[self.date_start:self.date_end]
    ...
    def _load_org_factor(self, org_factor_path):
        try:
            factor = pd.read_parquet(org_factor_path)
            if 'timestamp' in factor.columns:
                factor.set_index('timestamp', inplace=True)
            factor.columns = [c.lower() for c in factor.columns]
            factor = factor.loc[self.date_start:self.date_end]
            return factor
        except:
            traceback.print_exc()
            return None
    ...
    def _load_factor(self, factor_name):
        neu = self.neu
        neu_lookback = self.params.get('neu_lookback')
        neu_data_dir = self.params.get('neu_data_dir')
        ...
        if neu is not None:
            ...
            # else:
            factor = self._load_org_factor(org_factor_path)
            if factor is None:
                return None
            to_mask = factor.isna()
            try:
            ...
            for pn, fn in neu.gradients:
                if pn == 'curr_price':
                    ...
                    neu_data_dir = Path(neu_data_dir) if neu_data_dir is not None else self.factor_data_dir
                    fn_path = neu_data_dir / pn / f'{fn}.parquet'
...
# =============================================================================
        ...
        # steps_end = np.arange(bin_step_sw, 1+bin_step_sw, bin_step_sw)
        steps_start = [0, 0, 0, 0.825, 0.85, 0.875]
        steps_end = [0.125, 0.15, 0.175, 1, 1, 1]
        steps_of_lag_sw = {}
        ...
        for lag in lag_list:
            ...
            for step_start, step_end in zip(steps_start, steps_end):
                ...
                ps_step = pd.DataFrame(np.where(selected_range, 1, 0), 
                                       index=factor_rank.index, columns=factor_rank.columns).fillna(method='ffill').fillna(0)
                w_step = ps_step != 0
                rtn_step = (rtn_lag[w_step]).mean(axis=1).shift(lag).fillna(0)
                rtn_step_d = rtn_step.resample('D').sum(min_count=1).dropna()
                df_step[f'{step_start:.1%} - {step_end:.1%}'] = rtn_step_d
...
#         # bins vol penalty
#         bin_step_vp = 0.1
#         steps_start = np.arange(0, 1, bin_step_vp)
#         steps_end = np.arange(bin_step_vp, 1+bin_step_vp, bin_step_vp)
#         steps_of_lag_vp = {}
#         
...
# 
#         # rtn_vol = rtn_1p.shift(1).rolling(window=48).apply(cumulative_volatility, raw=True)
#         rtn_1sp = (twap_price / twap_price.shift(1) - 1).replace([np.inf, -np.inf], np.nan)
#         rtn_vol = rtn_1sp.rolling(window=48).apply(cumulative_volatility, raw=True)
#         
#         for lag in lag_list:
#             rtn_lag = rtn_of_lag[lag]
#             df_step = pd.DataFrame()
...
#                 selected_range = (factor_rank > step_start) & (factor_rank <= step_end)
#                 ps_step = pd.DataFrame(np.where(selected_range, 1, 0), 
#                                        index=factor_rank.index, columns=factor_rank.columns).fillna(method='ffill')
#                 ps_step = ps_step.replace(0, np.nan)
#                 valid_vol_rank = (rtn_vol * ps_step).rank(axis=1, pct=True)
#                 need_penalty = (valid_vol_rank > 0.9).astype(int)
#                 ps_with_penalty = (ps_step - 0.5 * need_penalty).fillna(0)
#                 # ps_with_penalty = ps_step / rtn_vol
#                 w_step = ps_with_penalty.div(ps_with_penalty.sum(axis=1, min_count=1), axis=0).replace([np.inf, -np.inf], np.nan).fillna(0)
#                 rtn_step = (w_step * rtn_lag).sum(axis=1, min_count=1).shift(lag).fillna(0)
#                 rtn_step_d = rtn_step.resample('D').sum(min_count=1).dropna()
#                 df_step[f'{step_start:.0%} - {step_end:.0%}'] = rtn_step_d
#             steps_of_lag_vp[lag] = df_step
#         with open(self.data_dir / f'bins_vp_{factor_name}.pkl', 'wb') as file:
#             pickle.dump(steps_of_lag_vp, file)
# =============================================================================
...
#         steps_of_lag_vp = {}
#         
#         roc = (twap_price / twap_price.shift(8) - 1).shift(1).replace([np.inf, -np.inf], np.nan).abs()
#         
#         for lag in lag_list:
#             rtn_lag = rtn_of_lag[lag]
#             df_step = pd.DataFrame()
...
#                                        index=factor_rank.index, columns=factor_rank.columns).fillna(method='ffill')
#                 ps_step = ps_step.replace(0, np.nan)
#                 valid_vol_rank = (roc * ps_step).rank(axis=1, pct=True)
#                 need_penalty = (valid_vol_rank > 0.9).astype(int)
#                 ps_with_penalty = (ps_step - 0.5 * need_penalty).fillna(0)
#                 # ps_with_penalty = ps_step / rtn_vol
#                 w_step = ps_with_penalty.div(ps_with_penalty.sum(axis=1, min_count=1), axis=0).replace([np.inf, -np.inf], np.nan).fillna(0)
...
#         ax4 = fig.add_subplot(spec[:11, -11:])
#         # ax4.set_title('bin_diff', fontsize=FONTSIZE_L2, pad=10)
#         bins_of_lag_0 = steps_of_lag_sw[0]
#         for i in range(10):
#             bin_to_long_up = 100 - i*5
#             bin_to_long_down = 100 - (i+1)*5
#             bin_to_short_up = (i+1)*5
#             bin_to_short_down = i*5
#             bin_to_long_name = f'{bin_to_long_down}% - {bin_to_long_up}%'
#             bin_to_short_name = f'{bin_to_short_down}% - {bin_to_short_up}%'
#             ax4.plot((bins_of_lag_0[bin_to_long_name] - bins_of_lag_0[bin_to_short_name]).cumsum(),
#                      linewidth=3, label=f'bin_{i}')
#         ax4.tick_params(axis='x', rotation=45)
# =============================================================================
...
#         bins_of_lag_0 = steps_of_lag[0]
#         ax4.plot((bins_of_lag_0['90% - 100%'] - bins_of_lag_0['0% - 10%']).cumsum(),
#                  color='k', linewidth=3, label='bin_diff_raw')
#         bins_vp_of_lag_0 = steps_of_lag_vp[0]
#         ax4.plot((bins_vp_of_lag_0['90% - 100%'] - bins_vp_of_lag_0['0% - 10%']).cumsum(),
#                  color='b', linewidth=3, label='bin_diff_vol_penalty')
#         ax4.tick_params(axis='x', rotation=45)
# =============================================================================
