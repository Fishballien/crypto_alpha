...
def backtest(test_name, backtest_name):
    ...
    # Add new parameters for simple version
    use_simple_version = backtest_param.get('use_simple_version', False)
    ma_window = backtest_param.get('ma_window', 8)  # Default MA window size
    ma_method = backtest_param.get('ma_method', 'ma')
    top_pct = backtest_param.get('top_pct', 0.1)    # Default top percentile
    bottom_pct = backtest_param.get('bottom_pct', 0.1)  # Default bottom percentile
    ...
    if funding_limit is not None:
        predict_funding_path = funding_dir / 'binance_usd_funding_rates_30min.parquet'
        predict_funding_rate = pd.read_parquet(predict_funding_path)
        predict_funding_rate = align_columns(main_columns, predict_funding_rate)
        predict_funding_rate
    ...
    # For simple version, precompute all positions
    if use_simple_version:
        # Calculate moving average of predictions
        if ma_method == 'ma':
            predict_ma = predict_res.rolling(window=ma_window, min_periods=1).mean()
        elif ma_method == 'ewma':
            predict_ma = predict_res.ewm(span=ma_window, adjust=False).mean()
        
        # Calculate rank
        factor_rank = predict_ma.rank(axis=1, pct=True).sub(0.5 / predict_ma.count(axis=1), axis=0).replace([np.inf, -np.inf], np.nan)
        
        # Create position dataframe based on rank
        # Long positions for top percentile
        top_positions = (factor_rank > (1 - top_pct)).astype(float)
        
        # Short positions for bottom percentile
        bottom_positions = (factor_rank <= bottom_pct).astype(float)
        ...
        simple_positions = scaled_top + scaled_bottom
        
        # Save simple positions for reference
        merge_and_save(simple_positions, backtest_dir / f"simple_pos_{name_to_save}.csv")
        
        # For optimization version, compute and save factor
        factor_rank = predict_res.rank(axis=1, pct=True).sub(0.5 / predict_res.count(axis=1), axis=0).replace([np.inf, -np.inf], np.nan)
        ...
        # Existing approach for optimization version
        factor_rank = predict_res.rank(axis=1, pct=True).sub(0.5 / predict_res.count(axis=1), axis=0).replace([np.inf, -np.inf], np.nan)
        fct_n_pct = 2 * (factor_rank - 0.5)
        factor = fct_n_pct.div(fct_n_pct.abs().sum(axis=1), axis=0).fillna(0)
        merge_and_save(factor, backtest_dir / f"factor_{name_to_save}.csv")
    ...
    if pre_w is None:
        if use_simple_version:
            # Get the first valid position from the simple strategy
            w0 = simple_positions.iloc[0]
        else:
    ...
    if not use_simple_version:
        # Only set up optimization function if not using simple version
    ...
    for idx, t in enumerate(tqdm(factor.index, desc='simulating trades')):
        ...
        try:
            if use_simple_version:
                # For simple version, just grab the pre-computed position
                w1 = simple_positions.loc[t]
                max_wgt_current = max_wgt  # Use default max weight
            else:
                # Original optimization approach
                ...
                # 保存调试数据（在优化之前）
                if t in [pd.to_datetime('2025-06-05 00:30:00'), pd.to_datetime('2025-06-05 01:00:00')]:
                    available_symbols = factor.columns[if_to_trade].tolist()
                    debug_data = {
                        'available_symbols': available_symbols,
                        'mm_t': {k: v[if_to_trade] for k, v in mm_t.items()},  # 只保存可交易的momentum数据
                        'his_pft_t': {k: v[if_to_trade] for k, v in his_pft_t.items()},  # 只保存可交易的历史收益数据
                        'current_pos': w0.copy() if hasattr(w0, 'copy') else w0,
                        'alpha': pd.Series(alpha, index=factor.columns),
                    }
                    save_backtest_debug_data(t, debug_data, backtest_dir)
        
                if t - last_big_move_t > timedelta(days=big_move_days):
                    w1 = alpha / np.sum(np.abs(alpha))
                    last_big_move_t = t
                else:
                    w1_opt, status, max_wgt_current = opt_func(alpha, w0_opt, if_to_trade, tradv_t, mm_t, his_pft_t, rtn_vol_t, 
            ...
            if funding_limit is not None:
                predict_funding_rate_t = predict_funding_rate.loc[t].values
                to_avoid_trade = np.sign(w1_opt) * predict_funding_rate_t > funding_limit
                w1 = np.where(to_avoid_trade, 0, w1_opt)
            else:
                w1 = w1_opt.copy()
            w_list.append(w1.copy())
            
            if use_simple_version:
                max_wgt_list.append(max_wgt_current)
            else:
                max_wgt_list.append(max_wgt_current)
