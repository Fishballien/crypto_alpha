...
from portfolio_management import future_optimal_weight_lp_cvxpy_with_var
...
test_name = 'merge_agg_240805_withbe'
backtest_name = 'to_015_maxmulti_2_lambda_3'  # mwgt_010 #_lqdt_500_mm_06
...
opt_func = partial(future_optimal_weight_lp_cvxpy_with_var, max_multi=max_multi, max_wgt=max_wgt, 
                   tradv_thresh=tradv_thresh, momentum_limits=momentum_limits,
                   _lambda=_lambda, epsilon=epsilon, max_iter=max_iter)
...
for idx, t in enumerate(tqdm(factor.index, desc='simulating trades')):
    ...
    try:
        ...
        valid_rtn = rtn_c2c.loc[:t, rtn_c2c.columns[if_to_trade]]
        his_cov_matrix = valid_rtn.cov().fillna(0).values
...
metrics = get_general_return_metrics(np.array(profd['return']))
